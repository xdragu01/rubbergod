...
class System(commands.Cog):
    ...
    async def unload(self, ctx: commands.Context, extension: str):
        if extension in self.unloadable_cogs:
            await ctx.send(
                utils.fill_message("cog_cannot_be_unloadable", cog=extension)
            )
            return
    ...
    async def on_command_error(self, ctx: commands.Context, error):
        ...
        if isinstance(error, commands.errors.CommandInvokeError):
            if isinstance(error.__cause__, commands.errors.ExtensionAlreadyLoaded):
                await ctx.send(
                    utils.fill_message("cog_is_loaded", cog=error.__cause__.name)
                )
            elif isinstance(error.__cause__, commands.errors.ExtensionNotLoaded):
                await ctx.send(
                    utils.fill_message("cog_is_unloaded", cog=error.__cause__.name)
                )
