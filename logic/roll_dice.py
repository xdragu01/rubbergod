...
class Roll:
    DICE_REGEX = (
        r"^\s*(?:(\d*)[dD](\d+)(?:(d[hl]?)(\d+))?" + r"(?:(k[hl]?)(\d+))?|(\d+))\s*$"
    )
    ...
    def single_roll_dice(self, match_result):
        ...
        if dice_count > Config.max_dice_at_once:
            raise SyntaxError(
                utils.fill_message(
                    "rd_too_many_dice_in_group", maximum=Config.max_dice_at_once
                )
            )
        ...
        if dice_sides > Config.max_dice_sides:
            raise SyntaxError(
                utils.fill_message(
                    "rd_too_many_dice_sides", maximum=Config.max_dice_sides
                )
            )
                    ...
                    if length > 0:
                        if 0 < to_skip < length:
                            if to_drop < length - to_skip:
                                crossed_indexes += lookup[i][
                                    to_skip : to_skip + to_drop
                                ]
                                to_drop = 0
                            else:
    ...
    def roll_dice(self, roll_string):
        ...
        if len(dice_groups) > Config.max_dice_groups:
            return utils.fill_message(
                "rd_too_many_dice_groups", maximum=Config.max_dice_groups
            )
