...
class Karma(BaseFeature):
    ...
    async def emoji_process_vote(self, channel, emoji):
        ...
        message += utils.fill_message(
            "karma_vote_info", delay=str(delay // 60), minimum=str(cfg.vote_minimum)
        )
        ...
        for server_emoji in message.guild.emojis:
            if not server_emoji.animated:
                ...
                if len(e) == 0:
                    ...
                    vote_value = await self.emoji_process_vote(
                        message.channel, server_emoji
                    )
    ...
    async def karma_give(self, message):
        ...
        if len(input_string) < 4:
            ...
            try:
                ...
                await message.channel.send(
                    utils.fill_message(
                        "karma_give_format_number", input=input_string[2]
                    )
                )
                return
    ...
    async def leaderboard(self, channel, action, order, start=1):
        ...
        for i, user in enumerate(board, start):
            ...
            if username is None:
                ...
                line = "{} – **{}**: {} pts\n".format(
                    i, username, getattr(user, column)
                )
            output += line
