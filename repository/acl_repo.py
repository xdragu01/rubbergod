...
class AclRepository(BaseRepository):
    def find_user_exception(self, member_id, target_id):
        user_exceptions = (
            ...
            .filter(Acl_user_binding.user_id == str(member_id))
            .all()
        )

        for exception in user_exceptions:
            result = self.find_target_exception(exception.acl_group_id, target_id)
    ...
    def find_target_exception(self, acl_group_id, target_id):
        ...
        while True:
            group_id = stack.pop()
            result = (
                session.query(Acl_rules)
                .filter(
                    Acl_rules.acl_snowflake == str(target_id),
                    Acl_rules.acl_group_id == group_id,
                )
                .one_or_none()
            )
            ...
            if result is None:
                groups = (
                    session.query(Acl_groups)
                    .filter(Acl_groups.parent_id == group_id)
                    .all()
                )
                for group in groups:
    ...
    def get_role_perms(self, role, target_id):
        role_exceptions = (
            ...
            .filter(Acl_role_binding.role_id == str(role.id))
            .all()
        )
        ...
        role_exceptions += (
            session.query(Acl_role_binding)
            .filter(
                Acl_role_binding.role_id == None,  # noqa
                Acl_role_binding.role_name == role.name,
            )
            .all()
        )
    ...
    def add_rule(self, acl_group_id, acl_snowflake):
        ...
        rule = Acl_rules(acl_group_id=acl_group_id, acl_snowflake=acl_snowflake)
        session.add(rule)
        session.commit()

    def edit_rule(self, id, acl_group_id, acl_snowflake):
        rule = Acl_rules(id=id, acl_group_id=acl_group_id, acl_snowflake=acl_snowflake)
        session.merge(rule)
        session.commit()
    ...
    def add_role(self, acl_group_id, role_id_name, perms):
        ...
        rule = Acl_role_binding(
            acl_group_id=acl_group_id, role_id=role_id, role_name=role_name, perms=perms
        )
        session.add(rule)
        session.commit()
    ...
    def del_role(self, id):
        session.query(Acl_role_binding).filter(Acl_role_binding.id == id).delete()
    ...
    def edit_user(self, id, acl_group_id, user_id, perms):
        rule = Acl_role_binding(
            id=id, acl_group_id=acl_group_id, user_id=user_id, perms=perms
        )
        session.merge(rule)
        session.commit()

    def del_user(self, id):
        session.query(Acl_user_binding).filter(Acl_user_binding.id == id).delete()
